...
singularity exec --nv -B /work -B $MY_TMPDIR --pwd "$WORK_DIR" /work/$(whoami)/4DSloMo.sif \
    /bin/bash -c "
        hf download moqiyinlun1/HiFiHuman --repo-type dataset --local-dir $MY_TMPDIR/datasets --include 'HiFi4G_Dataset/4K_Actor1_Greeting/*'\
        && cat $MY_TMPDIR/datasets/HiFi4G_Dataset/4K_Actor1_Greeting/4K_Actor1_Greeting.zip.part* > $MY_TMPDIR/datasets/4K_Actor1_Greeting.zip \
        && rm $MY_TMPDIR/datasets/HiFi4G_Dataset/4K_Actor1_Greeting/4K_Actor1_Greeting.zip.part* \
        && unzip -qo $MY_TMPDIR/datasets/4K_Actor1_Greeting.zip -d $MY_TMPDIR/datasets/4K_Actor1_Greeting \
        ...
        python render.py --model_path $MY_TMPDIR/output/4K_Actor1_Greeting_preprocess/ --loaded_pth=$MY_TMPDIR/output/4K_Actor1_Greeting_preprocess/chkpnt43750.pth --skip_video --time_duration -0.5 1.5 && \
        ...
        python render.py --model_path ./output/4K_Actor1_Greeting_preprocess_wan/ --loaded_pth=./output/4K_Actor1_Greeting_preprocess_wan/chkpnt43750.pth --skip_video --time_duration -0.5 1.5
    "
rm -rf $MY_TMPDIR
echo "End time: $(date)"